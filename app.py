...
def all_sales():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('SELECT v.*, u.nombre as cashier FROM ventas v JOIN usuarios u ON v.usuario_id = u.id ORDER BY v.fecha DESC')
    sales = cursor.fetchall()
    conn.close()
    for s in sales:
        s['subtotal'] = float(s['subtotal'])
...
def delete_sale(id):
    ...
    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('SELECT producto_id, cantidad FROM detalle_ventas WHERE venta_id = %s', (id,))
        detalles = cursor.fetchall()
        for d in detalles:
            cursor.execute('UPDATE productos SET cantidad_stock = cantidad_stock + %s WHERE id = %s', (d['cantidad'], d['producto_id']))
        cursor.execute('DELETE FROM detalle_ventas WHERE venta_id = %s', (id,))
...
@app.route('/api/sales/delete-all', methods=['DELETE'])
